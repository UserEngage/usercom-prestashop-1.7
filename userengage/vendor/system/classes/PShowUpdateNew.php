...
class PShowUpdateNew
{
    ...
    public static function getRecommendedProduct()
    {
        ...
        $products = @unserialize(
            file_get_contents(
                'https://modules.prestashow.pl/recommended_products/1', false, $context
            )
        );
    ...
    public function downloadUpdate()
    {
        ...
        if (!is_dir($tmppath)) {
            @mkdir($tmppath);
            if (!is_dir($tmppath)) {
                throw new \PShowUpdateException('Missing directory ' . $tmppath);
            }
        }

        if (!is_writable($this->getTmpPath())) {
            throw new \PShowUpdateException('Missing write permissions to ' . $tmppath);
        }

        if (!preg_match('/^[0-9]+\.[0-9]+\.[0-9]+$/', $this->getNewestVersionNumber())) {
            throw new \PShowUpdateException($this->getNewestVersionNumber());
        }
    ...
    public function extractUpdate()
    {
        ...
        if (!file_exists($this->getTmpPath() . $version . '.zip')) {
            throw new \PShowUpdateException('File not exists ' . $this->getTmpPath() . $version . '.zip');
        }
        ...
        if ($res === true) {
            $this->recurseRemoveDir($this->getModulePath() . 'classes/', false);
            $this->recurseRemoveDir($this->getModulePath() . 'override/', false);
            $this->recurseRemoveDir($this->getModulePath() . 'migrations/', false);

            $zip->extractTo($this->getTmpPath());
            $zip->close();

            // remove module class index to rebuild it
            @unlink($this->getModulePath() . 'module_class_index.php');

            return true;
        } else {
            throw new \PShowUpdateException(
                'Error "' . (string)$this->zipArchiveTranslateErrorCode($res) . '"'
                . ' while trying to open ' . $this->getTmpPath() . $version . '.zip'
            );
        }
    }
     ...
     * @return string
     */
    public function zipArchiveTranslateErrorCode($code)
    {
        switch ($code) {
            case 0:
                return 'No error';
    ...
    public function moveUpdateToModule()
    {
        ...
        @array_map("unlink", glob($this->getModulePath() . '/config*.xml'));
    ...
    public function recurseRemoveDir($_path, $removePath = false)
    {
        ...
        // protect files outside module directory
        if (stripos($path . '/', $this->getModulePath()) !== 0) {
            return false;
        }
    ...
    public function makeModuleBackup()
    {
        ...
        $destPath = $path . $this->getModuleName() . "_"
            . $this->getModuleVersionNumber() . "_" . date('H.i_d.m.Y') . ".zip";
        ...
        if (!is_dir($path)) {
            @mkdir($path);
            if (!is_dir($path)) {
                throw new \PShowUpdateException('Missing directory ' . $path);
            }
        }

        if (!$zip->open($destPath, ZipArchive::CREATE | ZipArchive::OVERWRITE)) {
            throw new \PShowUpdateException('Failed to init ZIP archive in ' . $destPath);
        }
        ...
        try {
            ...
            foreach ($files as $name => $file) {
                if (!$file->isDir()) {
                    ...
                    if (PShow_Settings::getInstance(__FILE__)->get('dont_keep_xml_in_backups')
                        && preg_match('/(.*)\.xml$/', $filePath)) {
                        continue;
                    }
    ...
    public function moveBackupToTmp($file)
    {
        if (!file_exists($this->getModulePath() . "update/backups/" . $file)) {
            throw new \PShowUpdateException(
                'File not exists ' . $this->getModulePath() . "update/backups/" . $file
            );
        }
        ...
        if (!file_exists($this->getTmpPath() . $file)) {
            throw new \PShowUpdateException(
                'Failed to copy file ' . $this->getModulePath() . "update/backups/" . $file
            );
        }
    ...
    {
        if (!file_exists($this->getTmpPath() . $file)) {
            throw new \PShowUpdateException('File not exists ' . $this->getTmpPath() . $file);
        }
        ...
        if ($res === TRUE) {
            ...
            throw new \PShowUpdateException('Failed to open ' . $this->getTmpPath() . $file);
        }
    }
    ...
    public function execUpdate($from_ver)
    {
        ...
        foreach ($instance->moduleVersionPath as $ver) {
            if (version_compare($from_ver, $ver . '.0', '>')) {
                continue;
            }

            $funcName = str_replace('.', '_', $ver);

            if (version_compare($instance->version, $ver . '.0') == 1 &&
                method_exists($instance, $funcName)) {
                $instance->{$funcName}();
            }
        }
